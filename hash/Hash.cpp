...
public:
  ...
  void move()
  {
    cout << left << setw(4) << x << setw(4) << y << setw(6) <<
      width << setw(6) << height << endl;
  }
...
public:
  ...
  void setup(int WIDTH, int HEIGHT, vector<Object*> list)
  {
    ...
    cout << "No. of objects: " << list.size() << endl;  
  ...
  void add(int object, vector<Object*> list)
  {
    ...
    //Top right
    int topright = (int) (floor((list[object]->x + list[object]->width) / cellsize) + floor((list[object]->y) / cellsize) * column);
             
    //Bottom left
    int bottomleft = (int) (floor((list[object]->x) / cellsize) + floor((list[object]->y + list[object]->height) / cellsize) * column);
    
    //Bottom right
    int bottomright = (int) (floor((list[object]->x + list[object]->width) / cellsize) + floor((list[object]->y + list[object]->height) / cellsize) * column);
    ...
    if(find(bucket[bottomleft].begin(), bucket[bottomleft].end(),
	    object) == bucket[bottomleft].end())
      bucket[bottomleft].push_back(object);
    if(find(bucket[bottomright].begin(), bucket[bottomright].end(),
	    object) == bucket[bottomright].end())
...
int main(int argc, char *argv[])
{
  ...
  for(unsigned int i = 0; i < size; i++)
    {
      int x = rand() % (SCREEN_WIDTH - Object::width*2) + Object::width; 
      int y = rand() % (SCREEN_HEIGHT - Object::height*2) + Object::height;
